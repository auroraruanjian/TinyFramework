 ...
 * 分页样式 
 * #p_page{font:12px/16px arial}
 * #p_page span{float:left;margin:0px 3px;}
 * #p_page a{float:left;margin:0 3px;border:1px solid #ddd;padding:3px 7px; text-decoration:none;color:#666}
 * #p_page a.now_page,#p_page a:hover{color:#fff;background:#05c}
*/
namespace core\lib;
...
class page
{
     ...
     * @param Array $data
     * $data = array(
            'total_rows'=> $count,              #(必须) 总页数
            'method'    => 'html',              #(必须) method 处理环境 设置为 html
            'parameter' => getUrl().'&p=??',    #(必须) ??问号的位置会自动替换为去向页码  为静态页面参数  xxx.com/20-0-0-0-40-??.html
            'now_page'  => 1,                   #(必须) 当前页
    ...
    public function show($param = 1)
    {
        ...
        {
            $return = "<div id='p_page'>".$this->$className()."</div>";
            return $return;
        }
        return '';
    }
